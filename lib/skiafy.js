...
export default function (svg, options = { translateX: 0, translateY: 0, scaleX: 1, scaleY: 1, outputEnd: false, outputColor: false }) {
        ...
        while (path) {
          ...
          if (isNaN(point)) {
            ...
            if (currentCommand.args.length == LengthForSvgDirective(svgDirective)) {
              commands.push({ 'command': SubsequentCommandForExtraArgs(svgDirective), 'args': [] });
